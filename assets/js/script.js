...
// main function to get data from local-storeage or API and print in organized way 
    ...
    const url = `/api/searcher?keyword=${encodeURIComponent(keyword)}`;
    ...
    try {
        ...
        if (data.results.length === 0) {
            ...
            data.results.forEach(movie => {
                let cardClone =
                    document.importNode(cardTemplate.content, true);  // Clone the template content

                cardClone.querySelector('.main__h1').innerText =
                    movie.title;

                cardClone.querySelector('.movieDate').innerText =
                    movie.release_date
                        ? movie.release_date.split('-')[0]
                        : "N/A";

                let rating =
                    cardClone.querySelector('.main__cardRating');
                ...
                cardClone.querySelector('.main__moviePoster').src =
                    `https://image.tmdb.org/t/p/w500${movie.poster_path}`;

                cardClone.querySelector('.main__moviePoster').alt =
                    `No Poster available for ${movie.title} Movie.`;
